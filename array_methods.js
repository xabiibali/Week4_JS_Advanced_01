...
runners.forEach(orod => {
const orodName = orod.first_name + ' ' + orod.last_name;
fullName.push(orodName)
})

console.log(fullName);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runner's first names converted to
// uppercase because the director BECAME DRUNK WITH POWER. Convert each first 
//name into all caps and log the result
let allCaps = [];

runners.map(big =>{
    const allbigs = big.first_name.toUpperCase()
    allCaps.push(allbigs);
...
//  Get a list of runners with large sized shirts so they can choose a different
// size. Return an array named largeShirts that contains information about the
// runners that have a shirt size of L and log the result
let largeShirts = runners.filter(shirt => shirt.shirt_size === 'L' );

console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes.
// Add up all the donations into a ticketPriceTotal variable and log the result

const ticketPriceTotal = runners.reduce((total, run) =>{
    return total + run.donation
},0 ) 
